...
function openAndInto() {
    ...
    sleep(2000)
    console.log('进入活动页面')
    let jdshouye = findTextDescMatchesTimeout(/首页/, 5000)
    if(jdshouye){
        console.log('现在在app首页')
        let huodong = desc("浮层活动").findOne(2000);
        //console.log(huodong)
        if (!huodong) {
            console.log('未能找到活动按钮。')
            back();
        }else{
            let bound = [huodong.bounds().centerX(), huodong.bounds().centerY()]
            console.log('活动按钮坐标',bound)
            click(bound[0],bound[1])
            sleep(6000)
        }
        
    }else if(className("android.widget.LinearLayout").depth(1).findOne()){
        console.log('已经在活动页面')
    }
    // app.startActivity({
    //     action: "VIEW",
...
function openTaskList() {
    console.log('打开任务列表')
    device.keepScreenDim(60 * 1000) // 防止息屏1分钟
    let jdshouye = findTextDescMatchesTimeout(/首页/, 1000)
    if(jdshouye){
        console.log('现在在app首页')
        let huodong = desc("浮层活动").findOne();
        //console.log(huodong)
        if (!huodong) {
            console.log('未能找到活动按钮。')
            back();
    ...
    let wxshouye = findTextDescMatchesTimeout(/通讯录/, 1000)
    if(wxshouye){
        console.log('现在在微信')
        back();
        sleep(3000)
...
function timeTask() {
    ...
    if (textMatches(/.*滑动浏览.*[^可]得.*/).findOne(10000)) {
        sleep(1000)
        console.log('模拟滑动')
        device.keepScreenDim(1 * 60 * 1000) 
...
function wxxTask() {
    sleep(3000)
    back();
    let wxshouye = findTextDescMatchesTimeout(/通讯录/, 2000)
    if(wxshouye){
        console.log('现在在微信')
        back();
        sleep(3000)
...
function wallTask() {
    console.log('进行品牌墙任务')
    sleep(5000)
    for (let i of [3, 6, 4, 5, 6]) { // 选5个
        console.log('打开一个')
        let pinpai = boundsInside(0, 2, device.width, device.height).clickable(true).findOnce(i).bounds();
        //let pinpai = boundsInside(0, 0, device.width, device.height).clickable(true).findOnce().bounds();
        //console.log('平拍',pinpai)
        let pinpaibound = [pinpai.centerX(), pinpai.centerY()]
         console.log('品牌位于', pinpaibound)
        click(pinpaibound[0], pinpaibound[1])
        //click(936,601)
        sleep(5000)
        console.log('直接返回')
        back()
...
function doTask(tButton, tText, tTitle) {
    ...
    if (tText.match(/浏览并关注.*s|浏览.*s/)) {
        ...
        console.log('进行入会任务')
        tFlag = joinTask()
    } else if (tText.match(/小程序/)) {
        let wx = tButton.bounds();
        let wxbut = [wx.centerX(), wx.centerY()]
        console.log('小程序位于', wxbut)
        click(wxbut[0], wxbut[1])
        console.log('进入小程序')
        tFlag = wxxTask()
        return true
